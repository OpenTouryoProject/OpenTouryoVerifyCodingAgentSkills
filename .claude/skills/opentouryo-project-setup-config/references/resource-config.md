...
**★ 既に別プロジェクトが `OT_RESOURCE_ROOT` を設定済みなら、奪い合わない。** ユーザ環境変数はマシンで1つなので、
別リポジトリの `resource\` を指したまま上書きすると**そのプロジェクトが壊れ**、新規の SQL/XML 定義がそちら側へ紛れ込む。
**既設なら別名にする**（例 `OT_RESOURCE_ROOT2`・またはプロジェクト固有名）：このプロジェクトの config 各キーと log4net の
`%env{...}%` を**その別名**に張り替える。どちらの名前を使ったかを `SETUP-CHANGES.md` に記録する。
