...
- **タイムスタンプ列が無い場合**：自動チェックが無いので、`D3_Update`／`D4_Delete`（動的 WHERE）に**取得時の全列の値**
  ...
  - **★ 前提＝全列を取得していること。** 生成 XML は各列が `<IF>AND [列] = @列<ELSE>AND [列] IS NULL</ELSE></IF>` なので、
    **パラメタを渡さない（扱っていない）列は `IS NULL` に落ち、非 NULL 列だと必ず0件**になる。一覧が全列でないなら
    D3/D4 の全列 WHERE は使えない → 全列取得にするか、**主キーのみ（`S3`/`S4`）＋件数0チェック**に割り切る。
